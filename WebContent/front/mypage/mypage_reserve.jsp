...
ArrayList<OrderMpInfo> myorderList = (ArrayList<OrderMpInfo>)request.getAttribute("myorderList");
...
<div class="main">
	...
	<div class="reserve">
		<h2>패키지 예약 내역</h2><hr />
		<p id="cancel" align="right">* 취소 요청 시 되돌릴 수 없습니다.</p>
		<table width="830" cellpadding="0" cellspacing="0" id="list">
			<tr>
				<th width="15%">예약 일자</th>
				<th width="15%">예약 번호</th>
				<th width="15%">상품명</th>
				<th width="15%">출발일/도착일</th>
				<th width="15%">가격</th>
				<th width="*">예약 현황</th>
				<th width="15%">예약 취소</th>
			</tr>
<%
if ((orderList.size() > 0 && flightList.size() > 0) ) {
	int num = rcnt - (psize * (cpage - 1));
	for (int i = 0; i < orderList.size(); i++) {
		OrderPaInfo op = orderList.get(i);	
		FlightInfo fi = flightList.get(i);
		
		String title = op.getOp_name();
		if (title.length() > 15)		title = title.substring(0, 13) + "...";
		title = "<a href=sdfsdfsd'?opcode=" + op.getOp_code() + args + "'>" + title + "</a>";
		String reserve = op.getOp_status();
		String cancelBtn = op.getOp_status();
...
%>
	<tr align="center">
		...
		<td width="15%"><%=fi.getFi_departure() %><br/><%=fi.getFi_entry() %></td>
		<td width="10%"><%=op.getOp_pay() %></td>
<% 		
		if (reserve.equals("a"))				reserve = "입금 대기중";
		else if (reserve.equals("b"))			reserve = "예약 완료";
		else if (reserve.equals("c"))			reserve = "예약 취소";
		else if (reserve.equals("d"))			reserve = "취소 완료";
%>
		<td width="*"><%=reserve %></td>
<%
		if (cancelBtn.equals("a")){
			cancelBtn = "입금 대기중";
			out.println("<td width='15%'>");
			out.println(cancelBtn);
			out.println("</td>");
		}
		else if (cancelBtn.equals("b")) {
			cancelBtn = "취소 요청";
			out.println("<td width='15%'>");
			//out.println("<a href='cancelform?opcode=" + op.getOp_code() + args + "'>" + cancelBtn + "</a>");
			out.println("<a href='cancelform?opcode=" + op.getOp_code() + "'>" + cancelBtn + "</a>");
			out.println("</td>");
		}
		else if (cancelBtn.equals("c") ){
			cancelBtn = "예약 취소 신청 중";
			out.println("<td width='15%'>");
			out.println(cancelBtn);
			out.println("</td>");
		}else if (cancelBtn.equals("d") ){
			cancelBtn = "예약 취소 완료";
			out.println("<td width='15%'>");
			out.println(cancelBtn);
			out.println("</td>");
...
if ((myorderList.size() > 0) ) {
	int num = rcnt - (psize * (cpage - 1));
	for (int i = 0; i < orderList.size(); i++) {
		OrderMpInfo om = myorderList.get(i);	
		
		String title = om.getOm_name();
		if (title.length() > 15)		title = title.substring(0, 13) + "...";
		title = "<a href=sdfsdfsd'?omcode=" + om.getOm_code() + args + "'>" + title + "</a>";
		String reserve = om.getOm_status();
		String cancelBtn = om.getOm_status();
		
%>
	<tr align="center">
		<td width="15%"><%=om.getOm_time() %></td>
		<td width="15%"><%=om.getOm_code() %></td>
		<td width="15%" align="left">&nbsp;&nbsp;<%=title %></td>
		<td width="15%"><%=om.getOm_leave() %><br/><%=om.getOm_arrive() %></td>
		<td width="10%"><%=om.getOm_pay() %></td>
<% 		
		if (reserve.equals("a"))				reserve = "입금 대기중";
		else if (reserve.equals("b"))			reserve = "예약 완료";
		else if (reserve.equals("c"))			reserve = "예약 취소";
		else if (reserve.equals("d"))			reserve = "예약 취소";
%>
		<td width="*"><%=reserve %></td>
<%
		if (cancelBtn.equals("a")){
			...
			cancelBtn = "취소 요청";
			out.println("<td width='15%'>");
			out.println("<a href='cancelmyform?omcode=" + om.getOm_code() + args + "'>" + cancelBtn + "</a>");
			//out.println("<a href='cancelmyform?omcode=" + om.getOm_code() + "'>" + cancelBtn + "</a>");
			out.println("</td>");
		}
		else if (cancelBtn.equals("c") ){
			cancelBtn = "예약 취소 신청 중";
